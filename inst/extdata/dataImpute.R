...
# 
# input = list.files("datasets", full.names = TRUE, pattern = "\\.csv|\\.rds")
...
# 
# input = list.files("datasets", full.names = TRUE, pattern = "\\.csv")
...
.libPaths(c("/home/Shared/Rlib/release-3.9-lib" ,
            "/usr/local/R/R-3.6.0/library",
            "/home/asonrel/R_lib36/"))
library(reticulate)
reticulate::use_condaenv("scVI_env", required = TRUE)

# devtools::load_all("../pipeComp-1/")
library(SingleCellExperiment)
library(Seurat)
library(scater)
setwd("/home/asonrel/projects/pipComp/")
source("/home/asonrel/projects/pipComp/pipeComp-1/inst/extdata/impWrappers.R")
library(purrr)

input = list.files("datasets", full.names = TRUE, pattern = "\\.csv|\\.rds")
out_dir <- "imputations/scVI/"
dir.create(out_dir, showWarnings = FALSE)
...
for (i in input) {
  ...
  input <- readRDS(i)
  res <- imp.scVI(input, 
                  py_script = "/home/asonrel/projects/pipComp/pipeComp-1/inst/extdata/scVI.py",
                  py_path = "/home/asonrel/miniconda3/envs/scVI_env/bin/python",) # <-------
