...
The `agg.by` argument let's you control for which of the parameters the values should be shown for each alternative (the values are averaged across the alternatives of other parameters). By default, TRP and FDR are here averaged across the three significance thresholds used in the evaluation, so we have created a pipeline-specific plotting wrapper which allows the selection of a significance threshold:

```{r}
dea_evalPlot_sig( res, what=c("TPR","FDR","logFC.pearson"), agg.by="sva.method", 
                  threshold=0.05 )
```
...
dea_evalPlot_sig(res, agg.by=c("sva.method","k"), 
